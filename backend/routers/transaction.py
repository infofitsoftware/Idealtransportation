...
from models.daily_expense import DailyExpense as DailyExpenseModel, DailyExpenseEntry as DailyExpenseEntryModel
...
from schemas.daily_expense import (
    DailyExpenseCreate, 
    DailyExpense, 
    DailyExpenseSummary,
    DailyExpenseEntry,
    DailyExpenseEntryCreate
)
...
@router.post("/daily-expenses", response_model=DailyExpense)
def create_daily_expense(
    ...
    try:
        ...
        logger.info(f"Expense data: {expense.dict()}")
        
        # Calculate total from entries
        total = sum(entry.amount for entry in expense.entries)
        
        # Create daily expense header
        db_expense = DailyExpenseModel(
            date=expense.date,
        ...
        for entry_data in expense.entries:
            db_entry = DailyExpenseEntryModel(
                daily_expense_id=db_expense.id,
                expense_type=entry_data.expense_type,
                sub_type=entry_data.sub_type,
                amount=entry_data.amount,
                location=entry_data.location,
                payment_mode=entry_data.payment_mode,
                receipt_url=entry_data.receipt_url,
        ...
        logger.info(f"Daily expense created successfully with ID: {db_expense.id}, {len(expense.entries)} entries")
        
        # Build response
        return DailyExpense(
            id=db_expense.id,
            date=db_expense.date,
            vehicle_number=db_expense.vehicle_number,
            ...
            entries=[
                DailyExpenseEntry(
                    id=entry.id,
                    daily_expense_id=entry.daily_expense_id,
        ...
        raise HTTPException(
            ...
            detail=f"Error creating daily expense: {str(e)}"
        )

@router.get("/daily-expenses", response_model=List[DailyExpense])
def get_daily_expenses(
    ...
    try:
        ...
        # Query expenses with entries - use joinedload to eager load entries
        from sqlalchemy.orm import joinedload
        expenses = db.query(DailyExpenseModel).options(
            joinedload(DailyExpenseModel.entries)
        ).filter(
            DailyExpenseModel.user_id == current_user.id
        ).order_by(DailyExpenseModel.date.desc()).all()
        ...
        for expense in expenses:
            try:
                # Safely access entries - handle case where entries might be None
                entries_list = expense.entries if expense.entries else []
                
                result.append(DailyExpense(
                    id=expense.id,
                    date=expense.date,
                    vehicle_number=expense.vehicle_number,
                    total=expense.total,
                    user_id=expense.user_id,
                    driver_name=current_user.full_name if current_user else None,
                    entries=[
                        DailyExpenseEntry(
                            id=entry.id,
                            daily_expense_id=entry.daily_expense_id,
        ...
        logger.info(f"Successfully processed {len(result)} expenses")
        return result
    except Exception as e:
        logger.error(f"Error in get_daily_expenses: {str(e)}", exc_info=True)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Error fetching daily expenses: {str(e)}"
        )
...
def get_daily_expense(
    ...
    logger.info(f"Fetching daily expense {expense_id} for user: {current_user.email}")
    from sqlalchemy.orm import joinedload
    expense = db.query(DailyExpenseModel).options(
        joinedload(DailyExpenseModel.entries)
    ).filter(
        DailyExpenseModel.id == expense_id,
        DailyExpenseModel.user_id == current_user.id
    ).first()
    if not expense:
    ...
    return DailyExpense(
        ...
        entries=[
            ...
            for entry in (expense.entries if expense.entries else [])
        ]
    )
